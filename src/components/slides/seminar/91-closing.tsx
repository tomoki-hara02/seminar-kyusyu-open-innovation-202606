...
/**
 * p91: クロージング — ご清聴ありがとうございました。
 *
 * 背景は `seminarSlides` の `background: 'logoParticles'`（ロゴパーティクル）。
 */

export default function Slide91Closing() {
  return (
    <SlideWrapper>
      <div className="relative z-10 flex flex-col items-center justify-center gap-8 md:gap-10 text-center w-full h-full max-w-4xl mx-auto px-6 md:px-8 pointer-events-none">
        ...
        <motion.p
          className="text-white/30 leading-relaxed tracking-wide"
          style={{ fontSize: 'clamp(10px, 0.8vw, 11px)' }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.7, delay: 0.95 }}
        >
          本資料は一般的な情報提供を目的とするものであり、個別の事案に関する法的助言ではありません。
          <br />
          具体的な案件については、専門家にご相談ください。
        </motion.p>
      </div>
    </SlideWrapper>
  );

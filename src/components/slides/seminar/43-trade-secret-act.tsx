...
/**
 * p43: 4-3 社内秘密情報・営業秘密 — 不正競争防止法 第2条6項
 *
...
function RiskBasedDecisionTree() {
  ...
  return (
    <motion.div
      className="flex flex-col gap-2 w-full shrink-0"
    ...
    >
      <div className="flex flex-col gap-0.5 shrink-0">
        ...
        <p
          ...
          style={{ fontSize: 'clamp(16px, 1.1vw, 19px)' }}
        >
          リスクベースアプローチの適用
        </p>
      </div>
      ...
      <div
        ...
        style={{ aspectRatio: '5 / 3', maxHeight: 'min(36vh, 300px)' }}
      >
        <svg
          viewBox="0 0 500 300"
...
function ManagementFlowColumn() {
  return (
    <motion.div
      className="flex flex-col gap-4 w-full min-h-0 self-start"
      initial={{ opacity: 0, x: 16 }}
      animate={{ opacity: 1, x: 0 }}
...
export default function Slide42TradeSecretAct() {
  return (
    <SlideWrapper>
      <motion.div
        className="flex flex-col gap-4 md:gap-5 w-full max-w-6xl pt-10 min-h-0 flex-1"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 flex-1 min-h-0 items-start overflow-y-auto scrollbar-thin">
          <div className="flex min-h-[20rem] md:min-h-[24rem] h-full w-full">
            <LegalQuoteColumn />
          </div>

        ...
        {AI_LOGIC_STEPS.map((step, i) => (
          <div key={step.label} className="flex md:flex-1 items-center gap-1.5 min-w-0">
            <div
              ...
              style={{
                color: step.highlight ? HIGHLIGHT_ACCENT : step.accent,
                borderColor: `${step.accent}${step.highlight ? '88' : '55'}`,
                background: step.highlight
                  ? `linear-gradient(135deg, ${HIGHLIGHT_ACCENT}18 0%, rgba(255,255,255,0.03) 100%)`
                  : `${step.accent}12`,
                fontSize: 'clamp(11px, 0.95vw, 14px)',
                boxShadow: step.highlight ? `0 0 20px -6px ${HIGHLIGHT_ACCENT}44` : undefined,
              }}
            >
              {step.label}
            </div>
...
export default function Slide59UnrecognizedCopyrightOutput() {
  return (
    <SlideWrapper>
      <motion.div

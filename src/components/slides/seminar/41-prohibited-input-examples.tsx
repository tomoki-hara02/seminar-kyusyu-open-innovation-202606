...
function CategoryCard({ item, index }: { item: ProhibitedCategory; index: number }) {
  return (
    <motion.div
      className="relative flex flex-col gap-2 p-3 md:p-3.5 rounded-xl border min-h-0 min-w-0 overflow-hidden"
      style={{
    ...
    >
      ...
      <div className="flex items-start gap-2.5 pl-2 min-w-0">
        <span
          ...
          style={{ color: item.accent, fontSize: 'clamp(10px, 0.9vw, 12px)' }}
        >
          {item.num}
        </span>
        <p
...
export default function Slide40ProhibitedInputExamples() {
  return (
    <SlideWrapper>
      <motion.div
        className="flex flex-col gap-4 md:gap-5 w-full h-full max-w-6xl px-2 py-4 pt-10 min-h-0"
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 md:gap-3 flex-1 min-h-0 auto-rows-fr overflow-y-auto scrollbar-thin">
